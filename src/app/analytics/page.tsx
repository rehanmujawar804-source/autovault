...
export default function AnalyticsPage() {
  ...
  const data = useMemo(() => {
    ...
    const totalProfit = filteredInvoices.reduce((sum, inv) => {
      return (
        sum +
        inv.items.reduce((iSum, item) => {
          const product = products.find((p) => p.id === item.productId);
          const buy = product?.buyPrice ?? 0;
          return iSum + (item.price - buy) * item.quantity;
        }, 0)
      );
    }, 0);
    ...
    filteredInvoices.forEach((inv) => {
      inv.items.forEach((item) => {
        ...
        const buyPrice = products.find((p) => p.id === item.productId)?.buyPrice ?? 0;
        productSalesMap[item.productId].qty += item.quantity;
        productSalesMap[item.productId].revenue += item.price * item.quantity;
        productSalesMap[item.productId].profit += (item.price - buyPrice) * item.quantity;
      });
    });
    ...
    const productMargins = products
      .map((p) => ({
        ...p,
        margin: p.sellPrice > 0 ? Math.round(((p.sellPrice - p.buyPrice) / p.sellPrice) * 100) : 0,
      }))
  ...
  const { salesLinePath, salesAreaPath, profitLinePath, profitAreaPath, trendPoints, maxChartVal } = useMemo(() => {
    ...
    // Populate trend values
    filteredInvoices.forEach((inv) => {
      const invProfit = inv.items.reduce((s, it) => {
        const p = state.products.find((prod) => prod.id === it.productId);
        return s + (it.price - (p?.buyPrice ?? 0)) * it.quantity;
      }, 0);
            ...
            <tbody className="divide-y divide-slate-100">
              {data.productMargins.map((p, i) => (
                <tr key={p.id} className="hover:bg-slate-50/70 transition-colors">
                  ...
                  <td className="px-5 py-2.5 text-right font-mono text-slate-500 text-xs">₹{p.buyPrice.toLocaleString()}</td>
