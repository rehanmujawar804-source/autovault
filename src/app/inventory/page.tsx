...
export default function InventoryPage() {
  ...
  function handleSave() {
    ...
    if (form.buyPrice < 0) { setFormError("Buy price cannot be negative."); return; }
    if (form.sellPrice <= 0) { setFormError("Sell price must be greater than 0."); return; }
    if (form.stock < 0) { setFormError("Stock cannot be negative."); return; }
    if (form.lowStockThreshold < 1) { setFormError("Low stock threshold must be at least 1."); return; }
  ...
  function handleImportCSV(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (event) => {
      ...
      for (let i = startRowIdx; i < lines.length; i++) {
        ...
        const existing = state.products.find((p) => p.sku.toLowerCase() === sku.toLowerCase());
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 divide-y md:divide-y-0 md:divide-x divide-slate-100">
          ...
          <div className="flex items-center gap-4 py-2 md:py-0 md:pr-4">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 font-bold text-sm ${
              insights.healthScore >= 90 ? 'bg-emerald-50 text-emerald-600' :
          ...
          <div className="flex items-center gap-4 py-2 md:py-0 md:pl-6">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
              insights.criticalCount > 0 ? 'bg-orange-50 text-orange-500 animate-pulse' : 'bg-slate-50 text-slate-400'
            }`}>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="flex gap-1.5 mt-1">
                                {outOfStock && (
                                  <span className="text-[9px] uppercase tracking-wider font-extrabold bg-red-50 text-red-600 border border-red-200 px-2 py-0.5 rounded">
                                    Out of Stock
                                  </span>
                                )}
                                {lowStock && (
                                  <span className="text-[9px] uppercase tracking-wider font-extrabold bg-orange-50 text-orange-600 border border-orange-200 px-2 py-0.5 rounded">
