...
export default function ProductDetailsPage({
  ...
  const capitalInvested = currentStock * buyPrice;
  ...
  // Create unified chronological timeline list
  const stockMovements = (() => {
    const list: Array<{
      date: string;
      type: "Sale" | "Audit" | "Import";
      delta: number;
      desc: string;
      reference: string;
      refUrl?: string;
    ...
    // Opening Stock event
    const creationDate = product.createdAt || new Date().toISOString();
    list.push({
      date: creationDate,
      type: "Import",
      delta: openingStock,
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3.5">
        ...
        {isOwner ? (
          <div className="bg-slate-50/50 border border-slate-200 p-4 rounded-2xl shadow-sm flex flex-col justify-between h-24">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Buy Price</span>
            <div className="flex items-baseline gap-0.5">
              <span className="text-xs font-black text-slate-400">₹</span>
              <span className="text-xl font-extrabold text-slate-700">{buyPrice.toLocaleString()}</span>
            </div>
          </div>
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-9 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 min-h-[360px]">
                ...
                <div className="py-12 flex flex-col items-center justify-center text-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5 max-w-xs leading-relaxed">
                      Purchase orders, supplier invoices, and delivery notes will appear here once the Supplier module is activated.
                    </p>
                  </div>
                </div>
              </div>
            ...
            {activeTab === "movement" && (
              <div className="space-y-6">
                <h3 className="text-base font-black text-slate-800">Dynamic Stock Ledger</h3>
